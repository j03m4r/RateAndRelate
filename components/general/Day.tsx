...
const Day: React.FC<DayProps> = ({
    ...
    return (
        <Tooltip content={prevDay.toDateString()}>
            <div onClick={rating!==-1&&mode==='profile' ? () => scrollIntoTheView(ratingId.toString()) : () => {}} 
            className={twMerge(`w-4 h-4 md:w-3 md:h-3 rounded-sm group`, rating==-1&&'bg-forestGreen opacity-10', mode==='profile'&&rating!==-1&&'cursor-pointer',
            dayNum===0 ? rowStart : null, rating===-1 ? 'hover:border hover:border-forestGreen' : rating>=7 ? 
